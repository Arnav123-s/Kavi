...
There are two cognitive layers: a learned template defining admissible roles and arrangements, and a learned circuit operating inside that boundary. The template represents the kinds of organization the system currently supports; it is not merely a fixed count of components. A separate device/search envelope supplies component cycles, update timing, encoding and work ceilings. Those physical implementation limits are not the learned world's ontology.

The first implementation of the learned boundary is [habitat.py](../kavi/phase/habitat.py). `Habitat` groups symbols by matching substitution contexts and records admitted role arrangements. `WorldGeneration` holds that learned template and the inner circuit. Teaching constructs both successors together, derives earlier obligations from the old configuration and admits the replacement only after validation. The [source experiment](../experiments/2026-09-08-pcl-habitat.md) learns two roles and then three while preserving its earlier answers. This is a finite input-role grammar; semantic entity categories, relations between persistent objects and world laws remain unimplemented.

The earlier [layers.py](../kavi/phase/layers.py) keeps its historical `CircuitTemplate` name for the supplied device/search envelope. Inner reconstruction samples inherited and new relationships across a complete definition and rebuilds the readout. Inheritance probability, candidate grammar, role-inference algorithm and update scheduling are still supplied. The learned boundary can remain unchanged while a circuit changes; template revision currently occurs at explicit teaching boundaries, not through a learned decision about when to update it.

Writing complete new objects is insufficient: the replacement must preserve earlier supported behavior. The habitat implementation checks the entire finite language admitted by its old template, within a 4,096-word expansion ceiling. This does not prove preservation for an unbounded world. Deliberate corrections can change explicitly disputed old targets. Active-state migration across a revision remains unimplemented; changes occur between invocations.
...
The template is the world's learned boundary and organizational rules; the reconstructed circuit supplies the executable organization within it. Both should ultimately be acquired, with the template changing less often. The implemented habitat takes the first finite structural step toward that distinction. The fixed execution envelope is separate. In the notation above, K carries the persistent learned organization and z its current activity. A particular situation need not become a stored episode, but K and z still require memory.
